...
public class ResolveBeforeEachParametersTest {
  ...
  @BeforeEach
  void beforeEach(LocalS3Endpoint endpoint, S3Client s3Client) {
    this.endpoint = endpoint;
    assertDoesNotThrow(() -> s3Client.createBucket(builder -> builder.bucket("my-bucket")));
  ...
  @Order(2)
  @Test
  void testInjectedInstancesShareTheSameEndpoint2(LocalS3Endpoint endpoint, S3Client s3Client) {
    assertEquals(this.endpoint, endpoint);
    assertDoesNotThrow(() -> s3Client.headBucket(builder -> builder.bucket("my-bucket")));
    assertDoesNotThrow(() -> s3Client.headBucket(builder -> builder.bucket("my-bucket")));
  }
  ...
  @AfterEach
  void afterEach(LocalS3Endpoint endpoint, S3Client client) {
    assertEquals(this.endpoint, endpoint);
